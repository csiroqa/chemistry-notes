\section{\S 2.1 Explain the Molecular Structure by VSEPR and Hybrid Orbital Theory}\label{sect:VSEPR和杂化轨道理论}
    ...
    \subsection{价层电子对互斥理论（VSEPR）}\label{subsec:VSEPR}
        ...
        \subsubsection{价层电子对的排列方式}\label{subsubsec:价层电子对排列方式}
            ...
            \begin{tabular}{|c|c|c|c|c|c|}
                \hline
                \textbf{价层电子对数} & \textbf{2 对} & \textbf{3 对} & \textbf{4 对} & \textbf{5 对} & \textbf{6 对} \\
                \hline
                \shortstack{VSEPR \\ 理论模型} & 直线形 & \shortstack{平面正\\三角形} & \shortstack{正四面\\体形} & \shortstack{（三角\\双锥形）} & \shortstack{（正八面\\体形）} \\
                \hline
                \shortstack{中心原子 \\ 杂化方式} & sp & sp\(^2\) & sp\(^3\) & \shortstack{sp\(^3\)d\\（三角双锥）} & \shortstack{sp\(^3\)d\(^2\)\\（正八面体）} \\
                \hline
            \end{tabular}
            \end{center}
        ...
        \subsubsection{确定价层电子对数}\label{subsubsec:确定价层电子对数}
            ...
            价层电子对数 =
            \[\underset{\text{成键}\upsigma\text{电子对数}}{n} + (\underset{\text{孤电子对数}}{\text{A 的价电子数}} - \overset{\makebox[0pt]{\footnotesize 代数值}}{\text{离子电荷}} - n \times \text{B 的单电子数})\div 2\]
        ...
        \subsubsection{实例分析}\label{subsubsec:VSEPR实例分析}
            ...
            \begin{tabular}{|c|c|c|c|c|c|}
                \hline
                \textbf{\shortstack{价层电子\\对数}} & \textbf{\shortstack{VSEPR\\模型}} & \textbf{\shortstack{成键\(\upsigma\)\\电子对数}} & \textbf{\shortstack{孤电子\\对数}} & \textbf{实例} & \textbf{分子几何构型} \\
                \hline
                \multirow{1}{*}{2} & 直线形 & 2 & 0 & \shortstack{\cemh{BeCl2}, \cemh{CO2}} & 直线形 \\
                \hline
                \multirow{2}{*}{3} & 正三角形 & 3 & 0 & \shortstack{\cemh{BF3}, \cemh{CO3^{2-}}} & 正三角形 \\
                \cline{3-6}
                & 三角形 & 2 & 1 & \cemh{SO2} & \shortstack{角形 \\\footnotesize（折线形、V 字形）} \\
                \hline
                \multirow{3}{*}{4} & 正四面体形 & 4 & 0 & \shortstack{\cemh{CCl4}, \cemh{CH4},\\\cemh{NH4+}} & 正四面体形 \\
                \cline{3-6}
                & 四面体形 & 3 & 1 & \shortstack{\cemh{NH3}, \cemh{PCl3},\\\cemh{PH3}} & 三角锥形 \\
                \cline{3-6}
                & 四面体形 & 2 & 2 & \shortstack{\cemh{H2O}, \cemh{H2S}} & \shortstack{角形 \\\footnotesize（折线形、V 字形）} \\
                \hline
            \end{tabular}
            \end{center}
            ...
            \noindent\textbf{总结}
            \begin{itemize}
                \item \textcolor{blue}{先计算出价层电子对总数，确定 VSEPR 模型；}
                \item \textcolor{blue}{若孤电子对数 = 0，则分子的空间构型即为 VSEPR 模型；}
                \item \textcolor{blue}{若孤电子对数 \(\neq\) 0，则从 VSEPR 模型扣除相应的顶点数，判断剩余原子组成的空间构型。}
            \end{itemize}
        ...
        \begin{pbm}{相同的 VSEPR 模型下键角不同的原因}
            都是 4 对价层电子对，相同的 VSEPR 模型，为什么键角不同？
        \end{pbm}
        ...
        \begin{xmp}{比较 \cemh{H2O}、\cemh{H2S}、\cemh{NH3}、\cemh{NF3} 的键角大小}
    ...
    \subsection{杂化轨道理论分析分子或复杂离子的结构}\label{subsec:杂化轨道理论}
        ...
        \begin{enumerate}
            ...
            \item 结合 VSEPR 模型和配位原子数分析空间结构。
            \item sp$^2$杂化，分析垂直于成键原子平面的 p 轨道如何形成\(\uppi\)键。
            \item sp 杂化，分析垂直于键轴的 p 轨道如何形成\(\uppi\)键。
            \item 若有多个原子的 p 轨道均垂直，则形成离域\(\uppi\)键。
        \end{enumerate}
        ...
        \subsubsection{实例：直线形分子}\label{subsubsec:直线形分子}
            ...
            \begin{minipage}[t]{0.48\textwidth}
                ...
                \cemh{CO2} 分子中含有 2 根\(\upsigma\)键、2 根\(\uppi\)键
        ...
        \subsubsection{实例：三角锥形分子}\label{subsubsec:三角锥形分子}
            ...
            \noindent VSEPR 模型：四面体形

            \noindent N 原子 sp$^3$ 杂化

            \noindent \(\therefore\) \cemh{NH3} 三角锥形
    ...
    \subsection{等电子体原理}\label{subsec:等电子体原理}
        ...
        \begin{tabular}{|c|l|}
            \hline
            \textbf{等电子体类别} & \textbf{实例} \\
            \hline
            3 原子 8 电子 & \cemh{H2O}、\cemh{H2S}、\cemh{NH2-} \\
            \hline
            4 原子 8 电子 & \cemh{NH3}、\cemh{PH3}、\cemh{H3O+}、\cemh{CH3-} \\
            \hline
            5 原子 8 电子 & \cemh{CH4}、\cemh{NH4+} \\
            \hline
            2 原子 10 电子 & \cemh{N2}、\cemh{CO}、\cemh{CN-}、\cemh{C2^{2-}}、\cemh{NO+} \\
            \hline
            3 原子 16 电子 & \cemh{CO2}、\cemh{CS2}、\cemh{N2O}、\cemh{BeCl2}、\cemh{NO2+} \\
            \hline
            3 原子 18 电子 & \cemh{SO2}、\cemh{O3}、\cemh{NO2-} \\
            \hline
            4 原子 24 电子 & \cemh{BF3}、\cemh{CO3^{2-}}、\cemh{NO3-}、\cemh{SO3} \\
            \hline
            4 原子 26 电子 & \cemh{PCl3}、\cemh{SO3^{2-}}、\cemh{ClO3^-}、\cemh{BrO3^-} \\
            \hline
            5 原子 32 电子 & \cemh{CF4}、\cemh{CCl4}、\cemh{SO4^{2-}}、\cemh{ClO4-}、\cemh{PO4^{3-}} \\
            \hline
        \end{tabular}
        \end{center}

        \begin{itemize}
            \item \cemh{PF5}：sp$^3$d 杂化，三角双锥
